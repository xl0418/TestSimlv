...
r <- c(1, 1, 1)
alpha <-
  matrix(c(0.2, 0.1, 0.3, 0.1, 0.5, 0.3, 0.1, 0.12, 0.3),
         nrow = 3,
         byrow = TRUE)
tmax <- 100
simresult <- simlv(
...
)) + geom_line(linewidth = 2) + theme_minimal() + xlab("Time") + ylab("Population size") + ggtitle("Simulated population dynamics") + scale_color_discrete(name = "Species")
...
L1 <-
  data.frame(
    processmodel = c(alpha),
    mvn_fit = c(b1_covariance),
    type = c("1,1", "1,2", "1,3", "2,1", "2,2", "2,3", "3,1", "3,2", "3,3")
  )

ggplot(L1, aes(x = mvn_fit, y = processmodel, color = type)) + geom_point() +
...
ggplot(
  data.frame(
    ...
    species = rep(c("X1", "X2", "X3"), each = nrow(simresult)),
    type = rep(c("observed", "fitted"), each = 3 * nrow(simresult))
  ),
  aes(
    x = time,
    y = values,
...
L2 <-
  data.frame(
    processmodel = c(alpha),
    mvn_fit = c(b2_covariance),
    type = c("1,1", "1,2", "1,3", "2,1", "2,2", "2,3", "3,1", "3,2", "3,3")
  )

ggplot(L2, aes(x = mvn_fit, y = processmodel, color = type)) + geom_point() +
  geom_smooth(color = "black", method = "lm")
...
f2 <-
  list(X1 ~ X1lag + X2lag + X3lag + 1,
       X2 ~ X1lag + X2lag + X3lag + 1,
       X3 ~ X1lag + X2lag + X3lag + 1)
...
L3 <-
  data.frame(
    processmodel = c(alpha),
    mvn_fit = c(b3_covariance),
    type = c("1,1", "1,2", "1,3", "2,1", "2,2", "2,3", "3,1", "3,2", "3,3")
  )

ggplot(L3, aes(x = mvn_fit, y = processmodel, color = type)) + geom_point() +
  geom_smooth(color = "black", method = "lm")
...
ggplot(L3x, aes(x = mvn_fit, y = processmodel, color = type)) + geom_point() +
  geom_smooth(color = "black", method = "lm")
